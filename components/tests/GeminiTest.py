...
class TestGemini(aiounittest.AsyncTestCase):
    ...
    def _compare_content_objects(self, content1: Content, content2: Content):
        return content1.role == content2.role and all(
            part1.text == part2.text
            for part1, part2 in zip(content1.parts, content2.parts)
        )
    ...
    def test_format_to_gemini_msg_format(self):
        ...
        self.assertTrue(
            all(
                self._compare_content_objects(expected_output[i], result[i])
