...
def generate_jwt(id_token: dict):
    ...
    encoded = jwt.encode({"authenticated": True, 
...
def generate_access_token(subject: str, expires_delta: int = None) -> str:
    if expires_delta is not None:
        ...
        expires_delta = datetime.utcnow() + timedelta(minutes=float(appconfig.ACCESS_TOKEN_EXPIRE_MINUTES))
...
def generate_refresh_token(subject: str, expires_delta: int = None) -> str:
    if expires_delta is not None:
        ...
        expires_delta = datetime.utcnow() + timedelta(minutes=float(appconfig.REFRESH_TOKEN_EXPIRE_MINUTES))

    to_encode = {"exp": expires_delta, "sub": str(subject)}
    encoded_jwt = jwt.encode(to_encode, appconfig.REFRESH_TOKEN_SECRET, appconfig.JWT_ALGORITHM)
    return encoded_jwt

def validate_token(token: str, type: str) -> bool: #TODO: add Annotation for type field description or use constants
...
def get_subject_refresh_token(token: str) -> str | None:
    try:
        payload = jwt.decode(token, appconfig.REFRESH_TOKEN_SECRET, algorithms=[appconfig.JWT_ALGORITHM])
